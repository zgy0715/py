...
def load_all():
    ...
    if sources.get("mongodb"):
        ...
        if collections:
            ...
            src_count = {}
            for coll_name, coll_info in (status.get("mongo_collections", {}) or {}).items():
                if coll_name == latest:
                    src_count = {s: 0 for s in (coll_info.get("sources") or [])}
                    break
            for src in (list(src_count.keys()) or ["arxiv", "hn", "reddit"]):
                if src == "github":
                    continue
                src_data = _fetch(
                    f"/api/hotspot?collection={latest}&source={src}&sort_by=hot_score&limit=300"
                )
                if src_data:
