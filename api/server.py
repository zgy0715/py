...
AI_ANALYSIS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "AI分析", "output")
...
@app.route("/api/ai-analysis/latest")
def ai_analysis_latest():
    """读取 AI分析/output/ 目录下最新的 insight JSON 文件"""
    if not os.path.exists(AI_ANALYSIS_DIR):
        os.makedirs(AI_ANALYSIS_DIR, exist_ok=True)
    json_files = [f for f in os.listdir(AI_ANALYSIS_DIR) if f.startswith("insight_") and f.endswith(".json")]
    if not json_files:
        return jsonify({"error": "暂无AI分析结果，请先运行 deepseek_analyzer.py 生成分析数据", "data": None})
    json_files.sort(reverse=True)
    latest_file = os.path.join(AI_ANALYSIS_DIR, json_files[0])
    try:
        with open(latest_file, encoding="utf-8") as f:
            data = json.load(f)
        data["_source_file"] = json_files[0]
        return jsonify({"error": None, "data": data})
    except Exception as e:
        return jsonify({"error": f"读取分析文件失败: {str(e)}", "data": None}), 500


@app.route("/api/ai-analysis/list")
def ai_analysis_list():
    """列出所有 AI 分析结果（仅元信息）"""
    if not os.path.exists(AI_ANALYSIS_DIR):
        return jsonify([])
    json_files = [f for f in os.listdir(AI_ANALYSIS_DIR) if f.startswith("insight_") and f.endswith(".json")]
    ...
    for fname in json_files:
        fpath = os.path.join(AI_ANALYSIS_DIR, fname)
        try:
            ...
            result.append({
                ...
                "projects_count": meta.get("projects_count", 0),
...
def ai_analysis_run():
    """直接导入并运行 deepseek_analyzer.py，返回最新结果"""
    import importlib.util, sys, glob, os as os_mod

    analyzer_path = os.path.join(
        os_mod.path.dirname(os_mod.path.dirname(os_mod.path.abspath(__file__))),
        "AI分析", "deepseek_analyzer.py"
    )
    if not os_mod.path.exists(analyzer_path):
        return jsonify({"error": "分析脚本不存在", "data": None}), 404

    try:
        spec = importlib.util.spec_from_file_location("deepseek_analyzer", analyzer_path)
    ...
    files = sorted(glob.glob(os.path.join(AI_ANALYSIS_DIR, "insight_*.json")), reverse=True)
    if not files:
        return jsonify({"error": "分析完成但未生成输出文件", "data": None}), 500
